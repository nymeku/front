...
export const AccountBlock: React.FC = () => {
	return (
		...
		>
			<Link className="hard-paragraphe" style={{ color: "gray" }} to={"/login?from=" + window.location.pathname}>
				Login/Signin
			</Link>
...
export const ProgressButton: React.FC<ProgressButtonProps> = ({ active, title, current, passed, onClick }) => {
	...
	return (
		<div
			className={(active ? "progress-button" : "") + " paragraphe"}
			onClick={onClick}
			style={{
				display: "flex",
				alignItems: "center",
...
const LocationBlock: React.FC<LocationBlockProps> = ({ location, change }) => {
	return (
		...
		>
			...
			<span className="hard-paragraphe">{location}</span>
		</div>
	)
}
